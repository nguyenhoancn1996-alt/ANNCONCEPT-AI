...
const BeforeAfterSlider: React.FC<{ before: string; after: string }> = ({ before, after }) => {
    ...
    return (
        ...
        >
            ...
            <div className="absolute inset-y-0 bg-white w-1 cursor-ew-resize shadow-sm" style={{ left: `${sliderPos}%` }}>
                <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-8 h-8 rounded-full bg-white flex items-center justify-center shadow-lg border border-slate-200">
                    <Icon name="arrows-pointing-out" className="w-4 h-4 text-slate-800 rotate-90" />
                </div>
            </div>
        </div>
    );
...
export const EditorBetaView: React.FC<{
  ...
  const handleBoxChange = useCallback((box: BoundingBox) => {
      ...
      if (ctx) {
        ...
        const mask = { base64, mimeType: 'image/png' };
    ...
    <div className="lg:col-span-12 grid grid-cols-1 lg:grid-cols-12 gap-6">
      ...
      <div className="lg:col-span-3 space-y-4">
        <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm">
          <h3 className="font-semibold text-slate-800 mb-3 text-lg border-b border-slate-200 pb-2">{t('editorBetaStep1')}</h3>
          <ImageDropzone onImageUpload={handleImageUpload} className='w-full border-2 border-dashed border-slate-300 bg-slate-50 hover:bg-slate-100 rounded-lg flex items-center justify-center text-center text-slate-500 text-sm cursor-pointer mb-4 min-h-[10rem] transition-colors'>
            {sourceImage ? 
          ...
          <label className="text-sm font-medium text-slate-600">{t('editorBetaSelectTool')}</label>
          <div className="flex bg-slate-100 rounded-md p-1 space-x-1 my-2 border border-slate-200">
            <button onClick={() => setTool('box')} className={`w-1/2 py-2 text-sm rounded transition-colors ${tool === 'box' ? 'bg-orange-600 text-white font-semibold' : 'text-slate-700 hover:bg-slate-200'}`}>{t('editorBetaBoundingBox')}</button>
            <button onClick={() => setTool('mask')} className={`w-1/2 py-2 text-sm rounded transition-colors ${tool === 'mask' ? 'bg-orange-600 text-white font-semibold' : 'text-slate-700 hover:bg-slate-200'}`}>{t('editorBetaMask')}</button>
          </div>
          {tool === 'mask' && (
            <div>
              <label htmlFor="brushSize" className="text-sm font-medium text-slate-600">{t('brushSize')}: {brushSize}px</label>
              <input id="brushSize" type="range" min="5" max="100" value={brushSize} onChange={(e) => setBrushSize(Number(e.target.value))} className="w-full h-2 bg-slate-300 rounded-lg appearance-none cursor-pointer accent-orange-500"/>
            </div>
          )}
        </div>
        
        <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm">
          <h3 className="font-semibold text-slate-800 mb-3 text-lg border-b border-slate-200 pb-2">{t('editorBetaStep2')}</h3>
          
          <div className="mb-3">
            <p className="text-xs text-slate-500 mb-1">{t('uploadReferenceOptional')}</p>
            {referenceImage ? (
                <div className="relative group">
                    <ImageDropzone onImageUpload={setReferenceImage} className="cursor-pointer rounded-lg"><div className='bg-slate-100 rounded-lg p-1 border border-slate-200'><img src={sourceImageToDataUrl(referenceImage)} alt="Ref" className="h-20 w-full object-contain rounded" /></div></ImageDropzone>
                    <button onClick={() => setReferenceImage(null)} className="absolute top-1 right-1 bg-white/80 rounded-full text-slate-800 hover:bg-white p-1 opacity-0 group-hover:opacity-100 z-10 shadow-sm"><Icon name="x-circle" className="w-4 h-4" /></button>
                </div>
            ) : (
                <ImageDropzone onImageUpload={setReferenceImage} className='w-full h-20 border-2 border-dashed border-slate-300 bg-slate-50 hover:bg-slate-100 rounded-lg flex items-center justify-center text-center text-slate-500 text-xs cursor-pointer transition-colors'>
                    <p>{t('dropzoneHint')}</p>
                </ImageDropzone>
            )}
          </div>
          ...
          <textarea
              ...
              className="w-full bg-slate-50 p-3 rounded-md h-24 resize-none text-sm focus:ring-2 focus:ring-orange-500 focus:outline-none border border-slate-300 text-slate-800 placeholder-slate-400"
          />
           <button onClick={handleGenerate} disabled={isLoading || !sourceImage || !selection} className="w-full bg-orange-600 hover:bg-orange-700 text-white font-bold py-3 px-4 rounded-lg flex items-center justify-center gap-2 disabled:bg-slate-200 disabled:text-slate-400 disabled:cursor-not-allowed mt-2 text-base shadow-sm">
         ...
         <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm">
          <h3 className="font-semibold text-slate-800 mb-3 text-lg border-b border-slate-200 pb-2">{t('editorBetaStep3')}</h3>
          <div>
              <label htmlFor="expansion" className="text-sm font-medium text-slate-600">{t('editorBetaExpansion')}: {expansion}px</label>
              <input id="expansion" type="range" min="0" max="100" value={expansion} onChange={(e) => setExpansion(Number(e.target.value))} className="w-full h-2 bg-slate-300 rounded-lg appearance-none cursor-pointer accent-orange-500"/>
            </div>
            <div className='mt-2'>
              <label htmlFor="edgeBlend" className="text-sm font-medium text-slate-600">{t('editorBetaEdgeBlend')}: {edgeBlend}px</label>
              <input id="edgeBlend" type="range" min="0" max="100" value={edgeBlend} onChange={(e) => setEdgeBlend(Number(e.target.value))} className="w-full h-2 bg-slate-300 rounded-lg appearance-none cursor-pointer accent-orange-500"/>
            </div>
            <button onClick={handleMerge} disabled={!finalResult || isLoading} className="w-full bg-orange-600 hover:bg-orange-700 text-white font-bold py-3 px-4 rounded-lg flex items-center justify-center gap-2 disabled:bg-slate-200 disabled:text-slate-400 disabled:cursor-not-allowed mt-4 text-base shadow-sm">
      ...
      <div className="lg:col-span-9 space-y-4">
        <div className="bg-slate-100 rounded-lg min-h-[40vh] flex items-center justify-center p-2 border border-slate-200 relative aspect-[4/3] shadow-inner">
          {sourceImage ? (
              ...
              {showMergedActions && (
                <div className="absolute top-4 right-4 z-20 flex flex-col gap-2">
                  <a 
                    ...
                    className="bg-white/80 backdrop-blur-sm border border-slate-300 hover:bg-white text-slate-800 p-2.5 rounded-full inline-flex shadow-sm" 
                  ...
                  <button 
                    ...
                    className="bg-white/80 backdrop-blur-sm border border-slate-300 hover:bg-white text-slate-800 p-2.5 rounded-full shadow-sm" 
                    title={t('useAsSource')}
                  >
                    <Icon name="arrow-up-tray" className="w-5 h-5" />
                  </button>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="bg-white p-3 rounded-xl border border-slate-200 shadow-sm aspect-video flex flex-col">
                <h3 className="font-semibold text-slate-800 mb-2 text-center flex-shrink-0">{t('editorBetaIntermediateResult')}</h3>
                <div className="flex-grow flex items-center justify-center min-h-0 bg-slate-50 rounded-lg">
                    {isLoading && <Icon name="sparkles" className="w-8 h-8 animate-spin text-orange-500" />}
                    {!isLoading && intermediateResult && selection?.box ? (
                        <BeforeAfterSlider 
                            before={sourceImageToDataUrl({ base64: sourceImage!.base64.split(',')[1] || sourceImage!.base64, mimeType: sourceImage!.mimeType })}
                            after={sourceImageToDataUrl(intermediateResult)}
                        />
                    ) : !isLoading && <p className="text-slate-400 text-sm">{t('emptyStateText')}</p>}
                </div>
            </div>
            <div className="bg-white p-3 rounded-xl border border-slate-200 shadow-sm aspect-video flex flex-col relative group">
                <h3 className="font-semibold text-slate-800 mb-2 text-center flex-shrink-0">{t('editorBetaFinalResult')}</h3>
                 <div className="flex-grow flex items-center justify-center min-h-0 bg-slate-50 rounded-lg">
                    {isLoading && <Icon name="sparkles" className="w-8 h-8 animate-spin text-orange-500" />}
                 ...
                 {!isLoading && finalResult && (
                    <div className="absolute top-2 right-2 flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                        <button onClick={() => setFullscreenImage(sourceImageToDataUrl(finalResult))} className="bg-white/80 backdrop-blur-sm border border-slate-300 hover:bg-white text-slate-800 p-2 rounded-md shadow-sm" title={t('fullscreen')}>
                            <Icon name="arrows-pointing-out" className="w-5 h-5" />
                        </button>
                        <a href={sourceImageToDataUrl(finalResult)} download={`aicomplex-edited-${Date.now()}.png`} className="bg-white/80 backdrop-blur-sm border border-slate-300 hover:bg-white text-slate-800 p-2 rounded-md shadow-sm" title={t('downloadImage')}>
                            <Icon name="download" className="w-5 h-5" />
                        </a>
                    </div>
                )}
