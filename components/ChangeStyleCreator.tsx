        ...
        <div className="bg-white p-5 rounded-xl border border-slate-200 shadow-lg animate-fade-in">
            <div className="flex items-center justify-between mb-8 border-b border-slate-200 pb-4">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <h2 className="text-2xl font-bold text-slate-800">{t('changeStyleTitle')}</h2>
                        <p className="text-sm text-slate-500">{t('changeStyleDesc')}</p>
                    </div>
                </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
                <div className="lg:col-span-4 space-y-6">
                    <section>
                        <h3 className="font-semibold text-slate-800 mb-3">{t('uploadImageForStyleChange')}</h3>
                        {sourceImage ? (
                          <div className='space-y-3'>
                              <ImageDropzone onImageUpload={setSourceImage} className="cursor-pointer rounded-lg">
                                <div className='bg-slate-100 rounded-lg p-2 border border-slate-200'><img src={sourceImageToDataUrl(sourceImage)} alt="Source for style change" className="w-full h-auto object-contain rounded" /></div>
                              </ImageDropzone>
                    ...
                    <section>
                         <h3 className="font-semibold text-slate-800 mb-2">{t('enterStyleRequest')}</h3>
                         <textarea
                            value={userPrompt}
                            onChange={(e) => setUserPrompt(e.target.value)}
                            placeholder={t('styleRequestPlaceholder')}
                            className="w-full bg-slate-50 p-3 rounded-md h-24 resize-none text-sm focus:ring-2 focus:ring-orange-500 focus:outline-none border border-slate-300 text-slate-900 placeholder-slate-400"
                        />
                        <button onClick={handleStylePromptGeneration} disabled={isLoading || !sourceImage || !userPrompt} className="mt-2 w-full bg-slate-200 hover:bg-slate-300 text-slate-800 font-bold py-2.5 px-4 rounded-lg flex items-center justify-center gap-2 disabled:bg-slate-100 disabled:text-slate-400 disabled:cursor-not-allowed text-sm transition-colors">
                    ...
                    <section>
                        <h3 className="font-semibold text-slate-800 mb-2">{t('generatedPromptReady')}</h3>
                         <textarea
                    ...
                    <button onClick={handleStyleImageGeneration} disabled={isLoading || !sourceImage || !generatedPrompt} className="w-full bg-orange-600 hover:bg-orange-700 text-white font-bold py-3 px-4 rounded-lg flex items-center justify-center gap-2 disabled:bg-slate-300 disabled:cursor-not-allowed text-base shadow-sm">
                       ...
                       <div className="flex flex-col h-full w-full">
                            <div className="flex-grow flex items-center justify-center relative group bg-white rounded-lg overflow-hidden border border-slate-200">
                                ...
                                <div className="absolute top-1/2 right-4 -translate-y-1/2 flex flex-col items-center gap-3 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                                    ...
                                    <a href={selectedImage} download={`annconcept.ai-styled-${Date.now()}.png`} className="bg-white/80 backdrop-blur-sm border border-slate-300 hover:bg-white text-slate-800 p-2.5 rounded-full inline-flex shadow-sm" title={t('downloadImage')}>
                        ...
                        <div className="text-center text-slate-400">
                            ...
                            <h3 className="text-xl font-semibold text-slate-600">{t('changeStyleTitle')}</h3>
